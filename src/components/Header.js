...
import {classNames, Link, safePrefix, toUrl} from '../utils';
                            ...
                            <div id="menu">
                                <ul>
                                    ...
                                    {_.map(_.get(this.props, 'pageContext.site.data.menu.actions'), (action, action_idx) => (
                                        <li key={action_idx}><Link to={(_.get(action, 'url').startsWith('#') ? _.get(action, 'url') : safePrefix(toUrl(this.props.pageContext.pages, _.get(action, 'url'))))}>{_.get(action, 'label')}</Link></li>
                                    ))}
                                </ul>
                            </div>
                        </li>
